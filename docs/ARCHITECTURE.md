...
DBADV-05 writes each stage transactionally and independently: run lifecycle, request snapshot, Contract B/deterministic findings, every provider attempt, validation/repair outcome, fallback, and final Contract C. Later-stage failure does not roll back earlier audit rows; the run is marked `failed` or `completed_with_fallback` and remains queryable. `/api/v1/analyses`, `/api/v1/analyses/{analysisId}`, and `/api/v1/analyses/{analysisId}/audit` provide inspection. `/api/v1/analyses/{analysisId}/replay` creates a child run from the persisted Contract B without invoking adapters or mutating the parent. Alembic owns file-backed schema evolution; in-memory test databases are deliberately isolated.
